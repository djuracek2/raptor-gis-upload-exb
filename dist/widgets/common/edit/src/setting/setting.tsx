...
> {
  ...
  getAllLayersOptions = (): AdvancedSelectItem[] => {
    ...
    const { useMapWidgetIds } = this.props
    const useMapWidget = useMapWidgetIds?.[0]
    const allLayersDs = getMapAllLayers(useMapWidget)
    if (allLayersDs.length > 0) {
      allLayersDs.forEach(item => {
  ...
  getSnapLayers = (defaultSnapLayers: Immutable.ImmutableArray<string>): AdvancedSelectItem[] => {
    ...
    const useMapWidget = useMapWidgetIds?.[0]
    const allLayersDs = getMapAllLayers(useMapWidget)
    if (allLayersDs.length > 0 && defaultSnapLayers) {
      allLayersDs.forEach(item => {
        const dsJson = item.getDataSourceJson()
        if (defaultSnapLayers.includes(dsJson.id)) {
          res.push({
            value: dsJson.id,
            label: dsJson.sourceLabel
          })
  ...
  render () {
    const { showLayerPanel, changeModeConfirmOpen, importHintType } = this.state
    const { theme, config, useMapWidgetIds } = this.props
    const { layersConfig, editMode, selfSnapping, featureSnapping, defaultSelfEnabled, defaultFeatureEnabled, defaultSnapLayers, snapSettingMode } = config
    const isAttrMode = editMode === EditModeType.Attribute
