...
import { createByGroupQuery, createByGroupSeries, fetchFieldRange, getByGroupOrderFields, getAppropriateTimeUnit, getParsedOrderByField, isSupportSplitBy } from './utils'
...
import { getDefaultValueFormat, getSplitByFieldValues, getStatisticsType } from '../../../../../../../../utils/common/serial'
...
export const ByGroupData = (props: ByGroupDataProps): React.ReactElement => {
  ...
  const numericFields = React.useMemo(() => {
    let nFields = outFields || outStatistics?.map((outStatistic) => outStatistic.onStatisticField)?.filter(field => !!field)
  ...
  const orderFields = React.useMemo(() => getByGroupOrderFields(propQuery, splitByFieldValue, translate), [propQuery, splitByFieldValue, translate])
  ...
  const handleCategoryFieldChange = async (fields: ImmutableArray<string>): Promise<void> => {
    ...
    if (useTimeBinning) {
      try {
        ...
        const series = createByGroupSeries({ splitByField, splitByFieldValues, categoryField, statisticType, numericFields, propSeries, timeIntervalUnits }, dataSourceId)
  ...
  const handleStatisticTypeChange = async (statisticType: ChartStatisticType): Promise<void> => {
    ...
    const orderByFields = [`${categoryField} ASC`]
    const series = createByGroupSeries({ splitByField, splitByFieldValues, categoryField, statisticType, numericFields: _numericFields, propSeries, timeIntervalUnits }, dataSourceId)
  ...
  const handleNumericFieldsChange = async (numericFields: ImmutableArray<string>): Promise<void> => {
    const orderByFields = [`${categoryField} ASC`]
    const _splitByField = numericFields?.length !== 1 ? '' : splitByField
    const series = createByGroupSeries({ splitByField: _splitByField, splitByFieldValues, categoryField, statisticType, numericFields, propSeries, timeIntervalUnits }, dataSourceId)
  ...
  const handleSplitByFieldChange = (splitByField: string, values: ImmutableArray<number | string>): Promise<void> => {
    const orderByFields = [`${categoryField} ASC`]
    const series = createByGroupSeries({ splitByField, splitByFieldValues: values, categoryField, statisticType, numericFields, propSeries, timeIntervalUnits }, dataSourceId)
    const query = createByGroupQuery({ categoryField, splitByField, statisticType, numericFields }, orderByFields, pageSize)
    if (!series?.length) return
    const chartDataSource = propChartDataSource.set('query', query)
    onChange(series, { chartDataSource, query })
