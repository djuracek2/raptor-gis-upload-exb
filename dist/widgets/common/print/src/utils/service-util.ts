...
import { PrintServiceType, CIMMarkerNorthArrow, DEFAULT_TEMPLATE_INFO, PrintTemplateType, ItemInfoType, ReportTemplateTypes, ReportTypes, LayoutTypes, DEFAULT_COMMON_SETTING, DEFAULT_TEMPLATE_LABEL } from '../config'
...
*/
export async function initNewTemplateItem (config: IMConfig, template: PrintTemplateProperties, newTemplateId: string): Promise<PrintTemplateProperties> {
  ...
  const newLabel = getNewTemplateName(config)
...
function getNewTemplateName (config: IMConfig) {
  let index = 0
  let newLabel = DEFAULT_TEMPLATE_LABEL
  const printTemplate = config?.printTemplateType === PrintTemplateType.Customize ? config.printCustomTemplate?.asMutable({ deep: true }) : config.printOrgTemplate?.asMutable({ deep: true })
  printTemplate?.forEach(item => {
    if (item.label === newLabel) {
      index += 1
      newLabel = `${DEFAULT_TEMPLATE_LABEL} ${index}`
    }
  })
  return newLabel
}
