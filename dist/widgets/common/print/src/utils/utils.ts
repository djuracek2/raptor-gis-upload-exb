...
export async function getTemplateOrReportInfo (printServiceUrl: string, itemInfoType?: ItemInfoType, itemId?: string): Promise<any[]> {
  ...
  return getPrintServiceTaskUrl(printServiceUrl).then(urlInfo => {
    const { layoutTemplatesInfoRequestUrl, reportTemplatesInfoRequestUrl, reportTemplatesInfoTaskUrl, executionType, layoutTemplatesInfoTaskUrl } = urlInfo
...
function getPrintServiceTaskUrl (printServiceUrl: string): Promise<TemplateInfoUrlInfo> {
    ...
    return esriRequest(serverUrl, options).then(res => {
      ...
      tasks?.forEach(taskName => {
        if (taskName?.includes('Layout') || taskName?.includes('Layout')) {
          ...
          layoutTemplatesInfoRequestUrl = `${serverUrl}/${taskName}/${supportedOperations}`
        }
