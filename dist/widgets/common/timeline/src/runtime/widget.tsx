/** @jsx jsx */
import {
  React, type AllWidgetProps, jsx, type DataSource, ReactResizeDetector, MultipleDataSourceComponent, type IMDataSourceInfo, DataSourceStatus, utils,
...
import { getCalculatedTimeSettings, getInsideLayersFromWebmap, getTimeExtentByTzOffset, getTimeSettingsFromHonoredWebMap } from '../utils/utils'
...
const Widget = (props: TimelineProps) => {
  ...
  const areDssReady = React.useMemo((): boolean => {
    ...
    const useDsIds = (useDataSources || Immutable([])).map(ds => ds.mainDataSourceId).asMutable({ deep: true })
  ...
  const formatMessage = (id) => {
    return intl.formatMessage({ id: id, defaultMessage: defaultMessages[id] })
  }

  const noTimelineFromHonoredMap = dataSources && reactiveUtils && timeSettingsForRuntime === null
  ...
  if (!useDataSources || useDataSources.length === 0) {
    ...
    />
  } else if (noTimelineFromHonoredMap || isDateExtentError || isTimezoneData) {
    const noTlFromHonoredMap = formatMessage('noTlFromHonoredMapWarning')
    const dateExtentError = formatMessage('invalidTimeSpanWarning')
    const timezoneWarning = formatMessage('timezoneWarning')
    return <div className='placeholder-container w-100 h-100 position-relative'>
      ...
      <Alert
        ...
        text={ noTimelineFromHonoredMap ? noTlFromHonoredMap : (isTimezoneData ? timezoneWarning : dateExtentError)}
      />
    </div>
  } else {
    return (
