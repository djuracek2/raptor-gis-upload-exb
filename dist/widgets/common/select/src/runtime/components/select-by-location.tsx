...
import {
  ...
  type DataSource, type ImmutableArray, DataSourceManager, dataSourceUtils, CONSTANTS
} from 'jimu-core'
import { defaultMessages as jimuUIMessages, Button, NumericInput, Select } from 'jimu-ui'
...
export default function SelectByLocation (props: SelectByLocationProps): React.ReactElement {
  ...
  return (
    <div className={classNames(['select-by-location p-4', { 'd-none': !visible }])} css={style}>
      ...
      {
        (featureCount >= 0) &&
        <div className='feature-count-info p-2 mt-3'>
          <InfoOutlined color='var(--info-600, #0077AC)' />
          <span className='pl-2' aria-label={selectByLocationFeatureCountTip}>{selectByLocationFeatureCountTip}</span>
        </div>
      }
    </div>
  )
...
async function getBufferedUnionGeometry (
  ...
  const queryResult = await dsExt.ds.queryAll({
    outFields: [],
    returnGeometry: true,
