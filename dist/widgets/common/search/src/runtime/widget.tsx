...
const Widget = (props: SearchProps) => {
  ...
  */
  const initGeocodeList = (configItem: NewDatasourceConfigItem) => {
    ...
    const { configId, hint, geocodeURL, outputDataSourceId, label, searchServiceType, singleLineFieldName, displayFields, defaultAddressFieldName, addressFields, isSupportSuggest, useUtility } = configItem
    const datasourceListItem = serviceList?.[configId]?.asMutable({ deep: true }) || {}
    const updateItem = {
      hint: hint,
