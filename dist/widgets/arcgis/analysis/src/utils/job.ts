import { buildProcessInfoReport, updateItemProperties, isEmptyDataItem, formatAnalysisEngineSuffix, getResultParams, generateRandomUniqueId } from 'analysis-shared-utils'
import { type ItemId, type AnalysisToolInfo, type LocaleItem } from 'analysis-ui-schema'
...
import { ClientJobIdPrefix } from 'analysis-core'
...
export async function executeJob (geoprocessor: __esri.geoprocessor, params: ExecuteProps, toolJson: AnalysisToolInfo, toolUrl: string, webToolServerUrl: string) {
  ...
  if (toolJson.executionType === 'esriExecutionTypeSynchronous') {
    ...
    try {
      const res: SynchronousJobExexuteResult = await geoprocessor.execute(toolUrl, jobParamsPayload)
