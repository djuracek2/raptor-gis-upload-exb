import { React, type ImmutableObject, MessageManager, DataSourcesChangeType, DataSourcesChangeMessage, type DataSource, DataSourceManager, getAppStore, esri, SessionManager, ServiceManager, css, ReactRedux, type IMState, type UtilitiesJson, type ImmutableArray } from 'jimu-core'
...
import { toCamelToolName, convertEsriMessageType, isEmptyDataItem, formatAnalysisEngineSuffix, canPerformStandardAnalysis, canPerformGeoAnalytics, canPerformRasterAnalysis, hasNAPrivilege, isStandardAnalysisServerAdvanced, type AnalysisHistoryItem, getRFxDefKeyFromRFT, type SerializedHistoryItem } from 'analysis-shared-utils'
import { AnalysisToolParamDataType, type AnalysisToolInfo, type GPFeatureRecordSetLayer, AnalysisEngine, type AnalysisToolItem, UserPrivileges, VisibilityModes, AnalysisType, type RFxTemplate, type AnalysisToolParam, AnalysisToolParamDirection } from 'analysis-ui-schema'
...
export const useGetDisplayedToolName = () => {
  ...
  return (toolInfo: ImmutableObject<ToolConfig> | ToolConfig) => {
    ...
    if (toolInfo.type === ToolType.Standard) {
      const displayedToolNameT9NKey = `${toCamelToolName(toolInfo.toolName)}${formatAnalysisEngineSuffix(
        toolInfo.analysisEngine
      )}`
      return toolInfoStrings?.[displayedToolNameT9NKey] as string || toolInfo.toolName
    }
