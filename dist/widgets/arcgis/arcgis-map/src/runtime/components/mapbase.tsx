...
export default class MapBase extends React.PureComponent<Props, State> {
  ...
  async getMapViewOptionToCreateMapView (dataSourceInfo: MapbaseDataSourceInfo): Promise<__esri.MapViewProperties> {
    ...
    if (this.props.isDefaultMap) {
      ...
      webMap = await this.createWebMapOrWebSceneByDataSource(dataSourceInfo) as __esri.WebMap
    ...
    try {
      ...
      const webmapConfig = await (webMap as any).resourceInfo

      if (webmapConfig) {
        (webMap as any).originalItemTimeZone = webmapConfig.timeZone
      }
    } catch (e) {
