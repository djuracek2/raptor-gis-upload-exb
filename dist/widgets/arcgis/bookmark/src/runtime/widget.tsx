...
import { emptyBookmarkListFromRuntime, getBookmarkListFromRuntime, getKey, getOldKey } from './utils/utils'
...
> {
  ...
  onViewBookmark = (item: ImmutableObject<Bookmark>, isRuntime?: boolean, index?: number) => {
    ...
    if (!isRuntime && index === this.state.highLightIndex) {
      BookmarkNodeList.forEach(node => {
        if (node.classList.contains(currentBookmarkId)) {
          const currentBookmarkNodes = node.querySelectorAll('.bookmark-container .bookmark-pointer,.bookmark-only-pointer,.bookmark-custom-pointer')
