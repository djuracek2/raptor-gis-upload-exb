...
import { React, LoadingType, type AllWidgetProps, getAppStore, jsx, SessionManager, type IMState, type UseUtility, UtilityManager } from 'jimu-core'
...
any> {
  ...
  async getGeoenrichmentServiceUrl () {
    this.geoenrichmentServiceUrl = await this.getUrlOfUseUtility(this.props.config.geoenrichmentConfig?.useUtility)
        ...
        {user?.username ? (
          <React.Fragment>
            {isPreset ? isPresetNotReady ? (
              <CustomWidgetPlaceholder iconSize='small' icon={theme.darkTheme ? baIconWhite : baIcon} label={this.localeString('_widgetLabel')} message={widgetPlaceholderTextToggle ? widgetPlaceholderText : ''} />
            ) : (
              <React.Fragment>
                {!showInfographicModal ? (
